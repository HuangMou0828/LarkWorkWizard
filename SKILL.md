...
当用户的指令中包含 **"dry-run"** 关键词时，进入 dry-run 模式：

- **所有查询操作正常执行**（`list_schedule`、`search_by_mql`、`get_workitem_brief` 等）
- **所有写入操作跳过**（`update_node`、`transition_node`）
- 正常展示计算结果和确认表格，但在表格末尾标注 `🔒 dry-run 模式，未实际执行`
- 不需要请求用户确认（因为不会真正写入）
...
🔒 dry-run 模式，未实际执行。去掉 "dry-run" 重新执行即可写入。
```
...
## 功能五：transition-today — 批量流转今日截止节点

> **明确语义**：本功能通过 `transition_node` MCP 调用，将今日排期截止的工作节点流转到下一个节点（如 FE开发 → FE联调）。不修改排期日期、不修改估分。
